...
**SHOULD I WRITE SOMETHING ABOUT THE STRUCTURE OF THE CHEAT SHEETS HERE? I'M STILL NOT EXACTLY SURE WHAT THE STRUCTURE SHOULD BE**
...
![Anyone getting this?](https://github.com/AguaClara/CEE4540_DC/blob/master/AguaClara%20Water%20Treatment%20Plant%20Design/Cheat%20Sheets/Images/Different%20forms%20of%20head.jpg?raw=true)
...
![I really don't think anyone can read this](https://github.com/AguaClara/CEE4540_DC/blob/master/AguaClara%20Water%20Treatment%20Plant%20Design/Cheat%20Sheets/Images/Minor%20loss%20pipe.jpg?raw=true)

$$\frac{p_{in}}{\rho g} + {z_{in}} + \frac{V_{in}^2}{2g} = \frac{p_{out}}{\rho g} + z_{out} + \frac{V_{out}^2}{2g} + h_L$$

Since the height of the 'in' and 'out' references are the same, we can eliminate $z_{in}$ and $z_{out}$. As we are considering such a small length of pipe, we will neglect the major losses component of head loss. As such, $h_L = h_e$. The following three equations are all the same, simply rearranged to solve for $h_e$.
...
This equation solved for $h_e$ has four variables, and we would like it to have only one. Thus, we will invoke conservation of momentum in the horizontal direction to remove some variables. The change in momentum from the 'in' point to the 'out' point is driven by the pressure difference between the points and shear along the wall. Note that we will be considering the pressure at the centroid of our control surfaces.
...
![What does this text do again?](https://github.com/AguaClara/CEE4540_DC/blob/master/AguaClara%20Water%20Treatment%20Plant%20Design/Cheat%20Sheets/Images/Hole%20in%20a%20bucket.jpg?raw=true)
